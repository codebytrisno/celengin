...
import { getCelengan, getTransactions, updateCelengan, deleteCelengan, createTransaction } from '@/lib/db'
...
export default function DetailCelenganWrapper() {
  return (
    <Suspense fallback={
...
function DetailCelenganPage() {
  const searchParams = useSearchParams()
  const id = searchParams.get('id')
  const router = useRouter()
  const { user, loading: authLoading } = useAuth()
  const [celengan, setCelengan] = useState<(import('@/lib/db').Celengan & { collected: number }) | null>(null)
  ...
  function updateCategory(newCategory: string) {
    if (!celengan || !id) return
    if (newCategory === celengan.category) {
      setEditingCategory(false)
      return
    }
    setSavingCategory(true)
    updateCelengan(id, { category: newCategory })
  ...
  if (!id) {
    return (
      <div className="flex min-h-screen bg-slate-50 dark:bg-slate-900">
        <main className="flex-1 overflow-x-hidden">
          <div className="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8 text-center py-20">
            <PiggyBank className="h-16 w-16 text-slate-300 dark:text-slate-600 mx-auto mb-4" />
            <p className="text-slate-500 dark:text-slate-400 font-medium text-lg">ID celengan tidak ditemukan</p>
            <Link href="/dashboard">
              <Button variant="clay" size="md" className="mt-4">Kembali ke Dashboard</Button>
            </Link>
          </div>
        </main>
      </div>
